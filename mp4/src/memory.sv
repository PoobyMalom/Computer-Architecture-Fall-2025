...
module memory #(
    ...
    logic [31:0] leds;              // Address 0xFFFFFFFC, R/W, four 8-bit PWM duty-cycle values for the user LED and the RGB LEDs
    logic [31:0] millis;            // Address 0xFFFFFFF8, R, count of milliseconds since processor started (mod 2^32)
    logic [31:0] micros;          // Address 0xFFFFFFF4, R, count of microseconds since processor started (mod 2^32)

    logic [$clog2(TICKS_PER_MILLISECOND) - 1:0] millis_counter;
    logic [$clog2(TICKS_PER_MICROSECOND) - 1:0] micros_counter;
    ...
    assign is_dmem = (dmem_address[31:12] == 20'd0) || (dmem_address[31:2] == 30'h3FFFFFFF);
